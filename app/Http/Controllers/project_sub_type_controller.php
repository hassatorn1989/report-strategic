...
class project_sub_type_controller extends Controller
{
    public function index(Request $request)
    {
        return view('project_sub_type');
    }

    public function lists(Request $request)
    {
        $q = tbl_project_sub_type::query();
        return DataTables::eloquent($q)
            ->filter(function ($q) use ($request) {
                if ($request->filter_project_sub_type_name != '') {
                    $q->whereRaw("project_sub_type_name like '%{$request->filter_project_sub_type_name}%'");
                }
            })
    ...
    {
        $request->validate([
            'project_sub_type_name' => 'required'
        ]);

        DB::beginTransaction();
        try {
            $q = new tbl_project_sub_type();
            $q->project_sub_type_name = $request->project_sub_type_name;
            $q->save();
            DB::commit();
    ...
    {
        $q = tbl_project_sub_type::find($request->id);
