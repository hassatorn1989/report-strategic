var table = $("#example1").DataTable({
    ...
    ajax: {
        url: myurl + "/setting/faculty/lists",
        type: "POST",
        data: function (d) {
            d.filter_faculty_name = $('input[name="filter_faculty_name"]').val();
        }
    },
    columns: [
...
function add_data() {
    ...
    $('#modal-default #form').attr('action', myurl + '/setting/faculty/store');
...
function edit_data(id) {
    ...
    $('#modal-default #form').attr('action', myurl + '/setting/faculty/update');
    $('#modal-default #form input[type="text"]').removeClass('is-invalid');
    $.ajax({
        type: "POST",
        url: myurl + '/setting/faculty/edit',
        data: {
            id: id
        },
        dataType: "json",
        success: function (response) {
            ...
            $('input[name="faculty_name"]').val(response.faculty_name);
        }
    });
}
...
function destroy(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: myurl + "/setting/faculty/destroy",
                data: {
                    id: id
                },
