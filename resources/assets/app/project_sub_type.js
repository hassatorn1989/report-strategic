var table = $("#example1").DataTable({
    ...
    ajax: {
        url: myurl + "/setting/project-sub-type/lists",
        type: "POST",
        data: function (d) {
            d.filter_project_sub_type_name = $('input[name="filter_project_sub_type_name"]').val();
        }
    },
    columns: [
        { data: null, sortable: false, searchable: false, className: "text-center" },
        { data: "project_sub_type_name", name: "project_sub_type_name" },
...
function add_data() {
    ...
    $('#modal-default #form').attr('action', myurl + '/setting/project-sub-type/store');
...
function edit_data(id) {
    ...
    $('#modal-default #form').attr('action', myurl + '/setting/project-sub-type/update');
    $('#modal-default #form input[type="text"]').removeClass('is-invalid');
    $.ajax({
        type: "POST",
        url: myurl + '/setting/project-sub-type/edit',
        data: {
            id: id
        },
        dataType: "json",
        success: function (response) {
            ...
            $('input[name="project_sub_type_name"]').val(response.project_sub_type_name);
        }
    });
}
...
function destroy(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: myurl + "/setting/project-sub-type/destroy",
                data: {
                    id: id
                },
