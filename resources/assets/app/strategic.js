var table = $("#example1").DataTable({
    ...
    ajax: {
        url: myurl + "/setting/strategic/lists",
        type: "POST",
        data: function (d) {
            d.filter_strategic_name = $('input[name="filter_strategic_name"]').val();
        }
    },
    columns: [
...
function add_data() {
    ...
    $('#modal-default #form').attr('action', myurl + '/setting/strategic/store');
...
function edit_data(id) {
    ...
    $('#modal-default #form').attr('action', myurl + '/setting/strategic/update');
    $('#modal-default #form input[type="text"]').removeClass('is-invalid');
    $.ajax({
        type: "POST",
        url: myurl + '/setting/strategic/edit',
        data: {
            id: id
        },
        dataType: "json",
        success: function (response) {
            ...
            $('input[name="strategic_name"]').val(response.strategic_name);
        }
    });
}
...
function destroy(id) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: myurl + "/setting/strategic/destroy",
                data: {
                    id: id
                },
